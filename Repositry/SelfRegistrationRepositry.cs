...
{
    public class SelfRegistrationRepositry
    {
        ...
        public SelfRegistrationRepositry(AppDbContext dbContext, JwtHandler _jwthand, IConfiguration configuration)
        {
        ...
        public async Task<dynamic> UpdateDailyTokenDetails(DailyToken details,string retal)
        {
            string[] parts = retal.Split('|');

            using (OracleConnection conn = new OracleConnection(con))
            {
                using (OracleCommand cmd = new OracleCommand("UCHTRANS.SP_APP_UPDATE_DLY_TKN_DTLS", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("P_PATIENT", OracleDbType.Varchar2).Value = details.Patient;
                    cmd.Parameters.Add("P_ADDRESS", OracleDbType.Varchar2).Value = details.Address;
                    cmd.Parameters.Add("P_SEX", OracleDbType.Varchar2).Value = details.Sex;
                    cmd.Parameters.Add("P_DOB", OracleDbType.Date).Value = details.Dob;
                    cmd.Parameters.Add("P_MOBILE", OracleDbType.Varchar2).Value = details.Mobile;
                    cmd.Parameters.Add("P_EMAIL", OracleDbType.Varchar2).Value = details.Email;
                    cmd.Parameters.Add("P_NATIONALITY_ID", OracleDbType.Varchar2).Value = details.NationalityId;
                    cmd.Parameters.Add("P_PATI_TYPE_ID", OracleDbType.Varchar2).Value = details.PatiTypeId;
                    cmd.Parameters.Add("P_PATI_MARITAL_STATUS", OracleDbType.Varchar2).Value = details.MaritalStatus;
                    cmd.Parameters.Add("P_TOKEN_ID", OracleDbType.Varchar2).Value = parts[0];
                    cmd.Parameters.Add("P_COUNTER_ID", OracleDbType.Varchar2).Value = details.CounterId;
                    cmd.Parameters.Add("P_AGE_YEAR", OracleDbType.Varchar2).Value = details.AgeYear;
                    cmd.Parameters.Add("P_IDCARD_ID", OracleDbType.Varchar2).Value = details.P_IDCARD_ID;
                    cmd.Parameters.Add("P_CUST_ID", OracleDbType.Varchar2).Value = details.P_CUST_ID;
                    cmd.Parameters.Add("P_IDCARD_NO", OracleDbType.Varchar2).Value = details.P_IDCARD_NO;
                    cmd.Parameters.Add("P_SALT_ID", OracleDbType.Varchar2).Value = details.P_SALT_ID;
                    ...
                    try
                    {
                        ...
                        return new { Status=200 , Message = "Registration Successfull",Token = parts};
                    }
                    catch (Exception ex)
                    {
                        return new { Status = 400, Message = ex.Message, Token = parts };
                    }
                }
            }
        }
        ...
        public async Task<dynamic> SaveDailyToken(string tokenSeries, string deviceId)
        {
            using (OracleConnection conn = new OracleConnection(con))
            {
                using (OracleCommand cmd = new OracleCommand("UCHTRANS.SP_APP_SAVE_DAILY_TKN", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    // Input parameters
                    cmd.Parameters.Add("P_TOKEN_SERIES", OracleDbType.Varchar2).Value = (object)tokenSeries ?? DBNull.Value;
                    cmd.Parameters.Add("P_DEVICE_ID", OracleDbType.Varchar2).Value = (object)deviceId ?? DBNull.Value;

                    // Output parameter
                    OracleParameter retval = new OracleParameter("RETVAL", OracleDbType.Varchar2, 100)
                    {
                        Direction = ParameterDirection.Output
                    };
                    cmd.Parameters.Add(retval);
                    ...
                    try
                    {
                        conn.Open();
                        cmd.ExecuteNonQuery();
                        string val =  retval.Value?.ToString() ?? "No return value";
                        //return await UpdateDailyTokenDetails(dt, val);
                        if (val == null)
                        {
                            return new { Status = 400, Message = "Unable to generate token" };
                        }
                        else
                        {
                            return new { Status = 200, Data = val };
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public async Task<DataTable> GetIdentityCards()
        {
            ...
            try
            {
                using (OracleConnection conn = new OracleConnection(con))
                {
                    string query = @"SELECT IDCARD_ID, IDCARD_NAME, DEFAULT_STS  
                                 FROM UCHMASTER.OPN_IDENTITY_CARD C  
                                 WHERE NVL(IDCARD_STATUS,'A')='A' 
                                 ORDER BY IDCARD_NAME ASC";
        ...
        public async Task<dynamic> GetPriorityTokensAsync(string strDeviceId)
        {
            ...
            try
            {
                using (var conn = new OracleConnection(con)) // _con is your connection string
                {
                    await conn.OpenAsync();

                    string query = @"
                SELECT P.*, CMP_NAME  
                FROM UCHMASTER.GEN_PRIORITY_TOKEN_TYPE P
                INNER JOIN UCHDISPLAY.DEVICE_TOKEN_TYPE_EXT E ON E.TOKEN_TYPE_ID = P.PTT_ID
                INNER JOIN UCHDISPLAY.DEVICE_EXT DE ON E.EXT_ID = DE.ID
                INNER JOIN UCHADMIN.SETTINGS S ON 1=1
                WHERE P.ACTIVE_STATUS = 'A' AND DE.DEVICE_ID = :DeviceId";

                    using (var cmd = new OracleCommand(query, conn))
                    {
                        cmd.Parameters.Add(new OracleParameter("DeviceId", OracleDbType.Varchar2)).Value = strDeviceId;
                        ...
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            ...
                            while (await reader.ReadAsync())
                            {
                                ...
                                {
                                    dict[col] = reader[col] == DBNull.Value ? null : reader[col];
                                }
